...
import { cn } from "@/services/strip-out/lib/utils";
  ...
  "group/cta relative inline-flex select-none items-center justify-center overflow-hidden rounded-sm font-semibold tracking-wide transition-all duration-400 ease-[cubic-bezier(0.22,1,0.36,1)] active:scale-[0.97] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:pointer-events-none disabled:opacity-60 motion-reduce:transform-none motion-reduce:transition-none",
  {
    variants: {
      variant: {
        ...
        /* Green button ON the green panel — the modal's form side.
           `solid` cannot be used there: it is `bg-primary`, the panel is
           `bg-primary`, and the button would vanish into it. This is the deeper
           green token with a white hairline ring, so it still reads as the same
           brand button while keeping an edge against the panel behind it. */
        solidOnGreen:
          "bg-primary-deep text-primary-foreground ring-1 ring-white/35 shadow-lg shadow-black/25 hover:ring-white/70 hover:shadow-xl hover:shadow-black/35",
      },
      size: {
...
const HOVER_TEXT_COLOR: Record<CtaVariant, string> = {
  ...
  solidOnGreen: "group-hover/cta:text-primary-foreground",
};
