 ...
 *
 * Kept deliberately small on a phone. It sits directly above the H1, and a white
 * pill on a dark photograph is the highest-contrast object in the hero — at any
 * real size it out-shouts the headline it is supposed to be introducing. Shrunk
 * to a credential the eye registers on the way past, the H1 gets to be the
 * loudest thing in the frame, which is the only thing that should be.
 *
 * The word "Google" is also dropped below `sm`. The four-colour G says it
 * perfectly well, and at 320px the full pill measured wider than the content box
 * and pushed the whole page sideways.
 */
export function GoogleRatingBadge({ className = "" }: { className?: string }) {
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-2.5 py-1 shadow-sm sm:gap-2.5 sm:px-3.5 sm:py-2 ${className}`}
    >
      <GoogleG className="h-3.5 w-3.5 sm:h-[18px] sm:w-[18px]" />
      <span className="text-[0.8rem] font-bold text-foreground sm:text-sm">{RATING}</span>
      <GoogleStars className="h-3 w-3 sm:h-[15px] sm:w-[15px]" />
      <span className="text-[0.7rem] whitespace-nowrap text-muted-foreground sm:text-xs">
