 ...
 *
 * Roughly half-height below `sm`. It is proof, not a headline — on a phone it
 * sits directly above the H1 and at full size it was competing with it for the
 * first glance, while the fold has no room to spare for a badge that only needs
 * to be believed, not read out. Every part of it steps down together: padding,
 * gap, the G, the stars and both pieces of type, so it shrinks as one object
 * rather than a pill with small text rattling around inside it.
 *
 * The word "Google" is dropped at that width too. Four items plus the word
 * measured about 290px, wider than the content box of a 320px phone — and the
 * four-colour G says "Google" perfectly well on its own.
 */
export function GoogleRatingBadge({ className = "" }: { className?: string }) {
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-2 py-1 shadow-sm sm:gap-2.5 sm:px-3.5 sm:py-2 ${className}`}
    >
      <GoogleG className="h-3.5 w-3.5 sm:h-[18px] sm:w-[18px]" />
      <span className="text-[0.72rem] font-bold text-foreground sm:text-sm">{RATING}</span>
      <GoogleStars className="h-[10px] w-[10px] sm:h-[15px] sm:w-[15px]" />
      <span className="text-[0.62rem] whitespace-nowrap text-muted-foreground sm:text-xs">
