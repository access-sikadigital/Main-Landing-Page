...
import { SITE, CTA_LABEL, quoteForm, quoteServiceOptions } from "@/services/rubbish-removal/lib/site-data";
import { quoteSchema, type QuoteValues } from "@/services/rubbish-removal/lib/quote-schema";
import { submitLead } from "@/services/rubbish-removal/lib/lead.functions";
import { captureTracking, getTracking } from "@/services/rubbish-removal/lib/tracking";
...
export function QuoteForm({
  ...
  const onSubmit = async (values: QuoteValues) => {
    ...
    try {
      ...
      navigate({ to: "/thank-you-rubbish-removal" });
    } catch {
      setErrorMsg(
            ...
            {comments && (
              <div className={WIDE}>
                ...
                <textarea
                  id="q-comments"
                  rows={4}
                  placeholder="Access, stairs, a long carry, anything heavy or awkward…"
                  className={inputClass}
                  {...register("comments")}
                />
              </div>
