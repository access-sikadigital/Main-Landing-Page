...
interface SectionHeadingProps {
  ...
  tone?: SectionTone;
  /**
   * Holds the description back until `sm`.
   *
   * For the descriptions that restate the title rather than add to it — on a
   * phone those are a line of text between the reader and the thing the section
   * is actually made of, and the eyebrow and title have already said it.
   */
  descriptionFromSm?: boolean;
}
      ...
      {description ? (
        <p
          className={`mt-3 text-[0.95rem] leading-relaxed sm:text-base ${
            descriptionFromSm ? "hidden sm:block" : ""
          } ${t.description}`}
        >
          {description}
        </p>
